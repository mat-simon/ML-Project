...
def graph_acc_trees(train_X, train_Y, validation_X, validation_Y):
    ...
    ax.plot(num_trees, train_scores, label="train", marker='o')
    ax.plot(num_trees, validation_scores, label="validation", marker='o')
    ax.legend()
...
def graph_acc_depth(train_X, train_Y, validation_X, validation_Y):
    ...
    best_acc = [0, 0]
    for i in num_depth:
        clf = RandomForestClassifier(n_estimators=3, max_depth=i, n_jobs=-1)
        clf.fit(train_X, train_Y)
        clfs.append(clf)
        score = clf.score(validation_X, validation_Y)*100
        if best_acc[1] < score:
    ...
    ax.set_ylabel("Accuracy %")
    ax.set_title("Max Depth vs Accuracy, fix n_estimators=3, max_features=100")
...
def graph_acc_features(train_X, train_Y, validation_X, validation_Y):
    ...
    ax.plot(max_features, train_scores, label="train", marker='o', markersize=2)
    ax.plot(max_features, validation_scores, label="validation", marker='o', markersize=2)
...
def train_model(train_X, train_Y, validation_X, validation_Y):
    ...
    """
    n_estimators_arr = list(range(3, 100)) #[x for x in range(1, 300, 1)]
    max_features = ['sqrt', 'log2', 1, 10, 20, 30, 40, 50, 60, 70, 80, 100]
...
def main():
    ...
    model = RandomForestClassifier(n_estimators=160, max_features="log2",
